from service import http_services
import service.envirocar_apis_service as envirocar_service
import json


# TODO put it in a service.
def write_to_file(track_id):
    track_response = http_services.request_json(
        envirocar_service.TRACKS_BASE_URL + "/{}".format(track_id))

    with open("data/{}.json".format(track_id), "w") as text_file:
...
def read_from_file_as_json(track_id):
    return json.loads(open("data/{}.json".format(track_id)).read())
...
if __name__ == '__main__':
    ...
    first_track = read_from_file_as_json(tracks_ids[0])
    first_track_features = first_track['features']

    # the most stupid analysis I can do is count "type": "feature" and "type": "other"
    # in this way, I can verify if there is another type.
